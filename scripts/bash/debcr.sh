#!/bin/bash
# Create the instance
qm create ${VM_ID-9000} -name debian-cloudinit -memory 1024 -net0 virtio,bridge=vmbr0 -cores 1 -sockets 1

# Import the OpenStack disk image to Proxmox storage
qm importdisk ${VM_ID-9000} debian-12-genericcloud-amd64.img local-lvm
...
qm set ${VM_ID-9000} --cicustom "user=local:snippets/user.yaml,network=local:snippets/network.yaml"
