...
# Install pre-reqs
install_apt_reqs() {
	sudo apt-get install -y fontconfig software-properties-common wget tar curl python3-venv git ninja-build gettext libtool libtool-bin autoconf automake cmake g++ pkg-config unzip curl doxygen
}
